...
- **Background music:** a single icon-only play/pause button plays one long, looping CC0 (public-domain) ambient track. It lives at the app shell, so playback continues across screens.
...
- `src/components/MusicPanel.tsx` / `src/lib/music.ts`: the background-music play/pause button and its track list (tracks served from `public/music/`).
