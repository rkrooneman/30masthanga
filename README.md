...
- **Generates a ~30-minute practice** that always opens with Sun Salutations (A ×3, B ×3) and closes with a shoulderstand and Savasana, keeps the canonical Primary Series order, and never mixes in poses from outside the series. The free time budget is spread across the standing (0.30), seated (0.40), and closing (0.30) sections, and one finishing pose (such as Headstand) is protected so a shortened practice always reaches a real close alongside the always-present shoulderstand. Generation is the starting point: the Overview then lets you customize the selection, adding poses beyond the 30-minute set if you want. "New sequence" wipes your current selection and generates a fresh set.
...
- **Practice overview (customizable selection):** a scannable, section-grouped map that shows every pose in the series in canonical order, with the poses not in your practice dimmed. Each card has a checkbox to include or exclude that pose, so the generated sequence is just a starting point you can tailor. A live running total (selected pose count and duration, per section and as a grand total at the bottom) updates as you check poses, and it is shown honestly even when adding poses pushes you past 30 minutes. The fixed frame is locked: Sun Salutations, Shoulderstand, and Savasana are always checked and cannot be removed. ×2 / ×3 markers show sides and traditional repeats. Tapping any card opens the swipeable detail carousel for studying that pose. The UI leads with Sanskrit: the map shows the Sanskrit name only, and the detail carousel shows Sanskrit as the primary name with English and phonetic as smaller secondary lines, alongside the pose's drishti (gaze).
- **Full series:** a "Full series" toggle on the Overview (next to "Basics only", and mutually exclusive with it) selects every pose at once; you can then uncheck any you want to skip. The choice is remembered on-device.
